...
class BookingService {
  ...
  async isSlotAvailable(courtId, date, startTime, endTime) {
    try {
      // Normalize date to midnight UTC to ensure consistent matching
      let dateObj = date;
      ...
      console.log('Normalized date for query:', dateObj.toISOString());
  ...
  async cancelBooking(bookingId, userId, reason) {
    try {
      ...
      const bookingDateTime = new Date(`${dateObj.toISOString().split('T')[0]}T${booking.startTime}`);
