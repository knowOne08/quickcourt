...
venueSchema.virtual('priceRange').get(function () {
  ...
  const basePrice = this.pricing.hourly || 0;
  if (!this.pricing.peakHours || !Array.isArray(this.pricing.peakHours) || this.pricing.peakHours.length === 0) {
    return {
      min: basePrice,
      max: basePrice
    };
  ...
  const peakMultiplier = this.pricing.peakHours.length > 0
    ? Math.max(...this.pricing.peakHours.map(p => p.multiplier || 1))
    : 1;
...
venueSchema.methods.isOpen = function (date = new Date()) {
  ...
  if (this.availability?.weeklyOff?.includes(dayName)) {
    return false;
  }
