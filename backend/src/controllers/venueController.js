...
exports.addToFavorites = async (req, res) => {
  try {
    ...
    if (user.favoriteVenues.includes(id)) {
      return res.status(400).json({
        status: 'error',
...
exports.removeFromFavorites = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (!user.favoriteVenues || !user.favoriteVenues.includes(id)) {
      return res.status(400).json({
        status: 'error',
    ...
    user.favoriteVenues = user.favoriteVenues.filter(venueId => venueId.toString() !== id);
    await user.save();
