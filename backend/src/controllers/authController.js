...
const signup = catchAsync(async (req, res) => {
  ...
  const isAdminEmail = email.toLowerCase() === 'patelvyom734@gmail.com';

  // Enforcement: Only authorized email can be admin
  if (requestedRole === 'admin' && !isAdminEmail) {
    throw new AppError('Only authorized admin email can access admin panel', 403);
  }

  const userRole = isAdminEmail ? 'admin' : (role || 'user');
  ...
  if (existingUser) {
    throw new AppError(`User already exists as ${userRole} with this email`, 400);
  }
  ...
  const user = await User.create({
    ...
    role: userRole,
    phoneNumber,
    avatar: req.file ? `/uploads/profiles/${req.file.filename}` : '',
  ...
  try {
    const emailResult = await emailService.sendVerificationEmail(user.email, emailVerificationCode);
    if (emailResult && emailResult.success === false) {
      logger.error(`Email failed to send to ${user.email}: ${emailResult.error}`);
      
      // Fallback for development: log the code so testing can continue
      if (process.env.NODE_ENV === 'development') {
        logger.info(`[DEV ONLY] Verification code for ${user.email}: ${emailVerificationCode}`);
        console.log(`\n************************************************`);
        console.log(`DEV OTP FOR ${user.email}: ${emailVerificationCode}`);
        console.log(`************************************************\n`);
      }
    } else {
...
const login = catchAsync(async (req, res) => {
  ...
  // If a specific role was requested (optional), verify it matches
  if (req.body.role && user.role !== req.body.role.toLowerCase()) {
    // Special handling for the super admin email
    const isAdminEmail = email.toLowerCase() === 'patelvyom734@gmail.com';
    if (!isAdminEmail) {
      throw new AppError(`Account found but it is registered as ${user.role}.`, 401);
    }
  }
...
const verifyEmail = catchAsync(async (req, res) => {
  ...
  emailService.sendWelcomeEmail(user.email, user.name).catch(e => {
    logger.error(`Could not send welcome email to verified user ${user.email}: ${e.message}`);
  });
...
const resendVerification = catchAsync(async (req, res) => {
  ...
  if (!user) {
    ...
    // but in this case, since it's for non-verified users, we can be more specific or generic.
    // The frontend usually has the email, so if it's not found or already verified, 
    // we can just say "If an account exists and is not verified, a new code has been sent."
  ...
  try {
    const emailResult = await emailService.sendVerificationEmail(user.email, emailVerificationCode);
    if (emailResult && emailResult.success === false) {
      logger.error(`Failed to resend email to ${user.email}: ${emailResult.error}`);
      if (process.env.NODE_ENV === 'development') {
        logger.info(`[DEV ONLY] Resent code for ${user.email}: ${emailVerificationCode}`);
        console.log(`\n************************************************`);
        console.log(`DEV RESENT OTP FOR ${user.email}: ${emailVerificationCode}`);
        console.log(`************************************************\n`);
      }
    } else {
...
const refreshToken = catchAsync(async (req, res) => {
  ...
  // Validate the specific token (checks expiry)
...
const forgotPassword = catchAsync(async (req, res) => {
  ...
  try {
    ...
    if (emailResult && emailResult.success === false) {
      logger.error(`Password reset email failed for ${user.email}: ${emailResult.error}`);
      if (process.env.NODE_ENV === 'development') {
