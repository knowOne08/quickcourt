...
export const useSecurePayment = () => {
  ...
  const generateDeviceFingerprint = useCallback(() => {
    try {
      ...
      const fingerprint = [
        navigator.userAgent,
        navigator.language,
        `${screenData.width || 0}x${screenData.height || 0}`,
  ...
  const createPaymentOrder = useCallback(async (bookingData) => {
    try {
      ...
      // If booking ID is missing, create a temporary booking token for backend booking creation.
      if (!bookingData.bookingId) {
        bookingData.bookingId = `booking_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
        console.log('Generated temporary bookingId for payment:', bookingData.bookingId);
      }
