...
import React, { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
// --- CHANGE 1: Imports for Stripe Elements added ---
...
// --- CHANGE 2: Stripe promise initialized outside the component ---
...
const BookingForm = ({ venueId, onSuccess }) => {
  ...
  const [formStep, setFormStep] = useState(1); // 1: Date, 2: Time, 3: Court, 4: Payment
  const [formLoading, setFormLoading] = useState(true);
  // --- CHANGE 3: State for clientSecret added ---
  ...
  useEffect(() => {
    ...
    fetchVenueDetails();
  }, [venueId, isAuthenticated, navigate]);

  const fetchVenueDetails = useCallback(async () => {
    try {
      setFormLoading(true);
      const response = await venueService.getVenueById(venueId);
      const venueData = response.data?.venue || response.data?.data?.venue || response.data;
      setVenue(venueData);
  ...
  const fetchAvailableSlots = useCallback(async () => {
    if (!selectedCourt?._id || !selectedDate || !duration) return;
    try {
      const response = await bookingService.getAvailableSlots(
        selectedCourt._id,
        selectedDate.toISOString().split('T')[0],
  ...
  // --- CHANGE 4: This function is now async and handles payment intent creation ---
  const handleContinueToPayment = async () => {
    // --- CHANGE: More robust validation ---
    if (!selectedDate || !selectedTimeSlot || !selectedCourt || !venue) {
      alert('Please ensure a date, time slot, and court are selected.');
      return;
    }

    // Recalculate the amount here to avoid using a stale state value.
    const currentPrice = selectedCourt.pricePerHour || 0;
    const currentHours = duration / 60;
    const currentTotalAmount = currentPrice * currentHours;

    if (currentTotalAmount <= 0) {
      alert('The booking amount must be greater than zero.');
      return;
    }
    ...
    // --- CHANGE: Add detailed logging to check the data before sending ---
    console.log("Submitting booking details:", bookingDetails);
    
    // Check for any null or undefined values before sending
    for (const key in bookingDetails) {
        if (bookingDetails[key] === null || bookingDetails[key] === undefined) {
            const errorMessage = `Error: '${key}' is missing from the booking details. Aborting payment.`;
            console.error(errorMessage, bookingDetails);
            alert(errorMessage);
            return;
        }
    }
    ...
    try {
      const bookingDetails = {
        amount: currentTotalAmount, // Use the freshly calculated amount
        currency: 'usd',
        venue: venue._id,
        court: selectedCourt._id,
        date: selectedDate.toISOString(), // Send date as a standardized ISO string
        startTime: selectedTimeSlot.startTime,
        endTime: selectedTimeSlot.endTime,
        duration,
        venueName: venue.name,
      ...
      const response = await paymentService.createPaymentIntent(bookingDetails);
      if (response.data.status === 'success') {
        setClientSecret(response.data.data.paymentIntent.client_secret);
        setFormStep(4);
      } else {
        throw new Error(response.data.message || 'Failed to prepare payment.');
      }
    } catch (error) {
      ...
      <div className="booking-form-content">
        {formStep === 1 && (
          <div className="form-step">
            ...
            <BookingCalendar selectedDate={selectedDate} onDateChange={handleDateChange} minDate={new Date()} />
          </div>
        )}
        ...
        {formStep === 2 && (
          <div className="form-step">
            ...
            <div className="court-selection">
              <div className="duration-selector">
                <label htmlFor="duration">Duration:</label>
                <select id="duration" value={duration} onChange={(e) => handleDurationChange(parseInt(e.target.value))} className="duration-select">
                  <option value={30}>30 minutes</option>
                  <option value={60}>1 hour</option>
                  <option value={90}>1.5 hours</option>
                  <option value={120}>2 hours</option>
              ...
              <div className="courts-grid">
                {venue.courts?.map(court => (
                  <div key={court._id} className={`court-option ${selectedCourt?._id === court._id ? 'selected' : ''}`} onClick={() => handleCourtChange(court)}>
                    <h4>{court.name}</h4>
                    <p>{court.sport}</p>
                    <span className="court-price">${court.pricePerHour}/hour</span>
                  </div>
            ...
            <div className="step-navigation">
              <button type="button" className="btn-secondary" onClick={() => goToStep(1)}>Back to Date</button>
              <button type="button" className="btn-primary" onClick={() => goToStep(3)} disabled={!selectedCourt}>Continue to Time Selection</button>
            </div>
          </div>
        )}
        ...
        {formStep === 3 && (
          <div className="form-step">
            ...
            <TimeSlotSelector availableSlots={availableSlots} selectedSlot={selectedTimeSlot} onSlotChange={handleTimeSlotChange} operatingHours={venue.operatingHours} selectedDuration={duration} />
            <div className="step-navigation">
              <button type="button" className="btn-secondary" onClick={() => goToStep(2)}>Back to Court Selection</button>
              <button type="button" className="btn-primary" onClick={handleContinueToPayment} disabled={!selectedTimeSlot || formLoading}>
                {formLoading ? 'Preparing Payment...' : 'Continue to Payment'}
              </button>
            </div>
          </div>
        )}

        {/* --- CHANGE 5: The Elements provider now wraps the PaymentSection --- */}
        {formStep === 4 && clientSecret && (
          <div className="form-step">
            <div className="step-header">
              <h2>Confirm & Pay</h2>
              <p>Review your booking details and proceed to payment</p>
            ...
            <div className="step-navigation">
              <button type="button" className="btn-secondary" onClick={() => goToStep(3)}>Back to Time Selection</button>
            </div>
          </div>
        )}
      </div>
